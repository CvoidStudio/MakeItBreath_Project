...
img1 = IMG.open(src)
x_scale = img1.size[0]/img1.size[1]
...
if len(Points) == 0:
	print('Sorry, no character detected by current MIB >_<\n')
	exit(0)
...
Final_PT = list([p[0],p[1],1,0,0] for p in Points)

U_Alpha = 7.0
D_Alpha = 10.0 - U_Alpha

Final_PT[37][2] = 3
Final_PT[37][3] = (Final_PT[41][0]-Final_PT[37][0])*U_Alpha
Final_PT[37][3] /= (float)(im_size[0])
Final_PT[37][4] = (Final_PT[41][1]-Final_PT[37][1])*U_Alpha
Final_PT[37][4] /= (float)(im_size[0])

Final_PT[36][2] = 2
Final_PT[36][3] = Final_PT[37][3]*0.2
Final_PT[36][4] = Final_PT[37][4]*0.2
...
for x in range(0,im_size[1],20):
	...
	# boarder
	if x < 64:
		x_shift *= SmoothStep(x,0,64)
	elif im_size[1]-x < 64:
		x_shift *= SmoothStep(im_size[1]-x,0,64)
		
	for y in range(0,FACES[0].top()-20,20):
		_Dist =  x_dist**2 + (y-Face_Center[1])**2
		y_shift = SmoothStep(_Dist,0,400)
		if y < 64:
			y_shift *= SmoothStep(y,0,64)
		Final_PT.append([x,y,0,x_shift,y_shift])
...
# Triangulation
delaunay_Tri = Triangulation(list(p[0] for p in Final_PT),list(p[1] for p in Final_PT)).triangles
